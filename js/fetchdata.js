...
function getData() {
    fetch(`${urlAPI}api/agenda/get-all`, requestOptions)
        ...
        .then(function (data) {
            ...
            const fs = require('fs');
            // fs.writeFile('agendaData.json', agendaData, (err) => {
            //     if (err) throw err;
    ...
    fetch(`${urlAPI}api/speaker/get-all`, requestOptions)
        ...
        .then(function (data) {
            ...
            localStorage.setItem('speakerData1', speakerData);
        });

    // fix undefined speaker avatar
    // var speakerData1 = JSON.parse(localStorage.getItem('speakerData'));

    // speakerData.forEach((speaker) => {
    //     if (speaker.avatar === undefined) {
    //         speaker.avatar = "default.png";
    //     }
    //     else {

    //         fetch(`${urlAPI}api/check-in/images/${speaker.avatar}`)
    //             .then(function (response) {
